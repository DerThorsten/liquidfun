...
protected:
    ...
    static void dealloc(instance<void> *self) {
        if (self->value) {
            ...
            bool isCasted = self->isCasted;
            if (!dont_cache && !isCasted) { // avoid an issue with internal references matching their parent's address
...
public:
    ...
    template <typename C, typename D, typename... Extra>
    class_ &def_readwrite_copy(const char *name, D C::*pm, const Extra&... extra) {
        cpp_function fget([pm](const C &c) -> const D &{ return c.*pm; },
    ...
    class_ & def_dynamic_cast(const char * name){
        auto castFunc = [](FROM * ptr)->TO *{
            auto cPtr = dynamic_cast<TO *>(ptr);
            if(cPtr == nullptr){
                pybind11_fail("def_dynamic_cast failed");
            }
            return cPtr;
        };

        this->def(name, castFunc,  return_value_policy::dont_cache_cast, keep_alive<1,0>());
        return *this;
    }
